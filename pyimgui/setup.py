...
if _CYTHONIZE_WITH_COVERAGE and not USE_CYTHON:
    raise RuntimeError(
        "Configured to build using Cython " "and coverage but Cython not available."
    )
...
EXTENSIONS = [
    ...
    Extension(
        "imgui.internal",
        extension_sources("imgui/internal"),
        extra_compile_args=os_specific_flags,
        libraries=os_specific_libraries,
        define_macros=[
            # note: for raising custom exceptions directly in ImGui code
...
setup(
    ...
    ext_modules=cythonize(
        EXTENSIONS, compiler_directives=compiler_directives, **cythonize_opts
    ),
