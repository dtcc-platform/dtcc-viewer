...
class PointCloudGL:
    ...
    def _create_shader(self):
        self.shader = compileProgram(compileShader(vertex_shader_pc, GL_VERTEX_SHADER), compileShader(fragment_shader_pc, GL_FRAGMENT_SHADER))
    ...
    def _get_instance_geometry(self, disc_size:float , n_points:int):
        ...
        if (n_points > self.upper_count):
            [self.vertices, self.face_indices] = self._create_quad(disc_size)   # Lowest resolution, only 2 triangles
        else:
            n_sides = self._calc_n_sides(n_points) 
