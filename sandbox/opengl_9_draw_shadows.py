...
glBufferData(GL_ARRAY_BUFFER, len(debug_vertices) * 4, debug_vertices, GL_STATIC_DRAW) # Second argument is nr of bytes

# Element buffer
EBO_debug = glGenBuffers(1)
glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO_debug)
glBufferData(GL_ELEMENT_ARRAY_BUFFER, len(debug_indices) * 4, debug_indices, GL_STATIC_DRAW)
...
shader_fancy = compileProgram(compileShader(vertex_shader_fancy, GL_VERTEX_SHADER), compileShader(fragment_shader_fancy, GL_FRAGMENT_SHADER))
...
shader_debug = compileProgram(compileShader(vertex_shader_debug, GL_VERTEX_SHADER), compileShader(fragment_shader_debug, GL_FRAGMENT_SHADER))
...
while not glfw.window_should_close(window):
    ...
    light_space_matrix = pyrr.matrix44.multiply(light_view, light_projection) # Other order? 
    glUseProgram(shader_shadow)
    light_space_matrix_loc = glGetUniformLocation(shader_shadow, "light_space_matrix")
    model_loc = glGetUniformLocation(shader_shadow, "model")
